...
router.put('/sites/:siteId', zValidator('json', z.object({
  ...
  settings: z.object({
    ...
    showAiPrompt: z.boolean().optional(),
...
// POST /admin/sites/:siteId/providers/:providerId/delete-comments — delete comments generated by this provider
router.post('/sites/:siteId/providers/:providerId/delete-comments', async (c) => {
  const db = getDb()
  const siteId = c.req.param('siteId')
  const providerId = c.req.param('providerId')
  requireSiteOwnership(c, siteId)

  const provider = db.select().from(providers).where(and(eq(providers.id, providerId), eq(providers.siteId, siteId))).get() as any
  if (!provider) throw new HTTPException(404, { message: 'Provider not found' })

  const result = db.delete(comments).where(and(
    eq(comments.siteId, siteId),
    eq(comments.providerName, provider.displayName)
  )).run()

  // Reset pageCache status back to pending to allow regeneration
  db.update(pageCache).set({
    status: 'pending',
    lockedAt: null,
    generatedAt: null,
    etag: null
  }).where(eq(pageCache.siteId, siteId)).run()
